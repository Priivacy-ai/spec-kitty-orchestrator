...
Coordinates multiple AI agents to autonomously implement and review work packages (WPs) in parallel. Integrates with spec-kitty via the versioned `orchestrator-api` CLI contract plus host-owned `agent status` / `agent tasks` commands where current host releases still require them for evidence-backed review handoff and authoritative task reconciliation.
